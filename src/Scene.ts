...
import { policy, provinceMap } from "./provinceMap";
...
export default class Scene extends PIXI.Container {
  ...
  constructor() {
    ...
    this.sceneMap.on("click_map", (data) => {
      ...
      var startPoint: Point = MapRoadUtils.instance.getWorldPointByPixel(
        this.player.x,
        this.player.y
      );
      var targetPoint: Point = MapRoadUtils.instance.getWorldPointByPixel(
        targetX,
        targetY
      );

      var startNode: RoadNode =
        this._roadDic[startPoint.x + "_" + startPoint.y];
      var targetNode: RoadNode =
        this._roadDic[targetPoint.x + "_" + targetPoint.y];
      console.log(startNode, targetNode);

      var roadNodeArr: RoadNode[] = this._roadSeeker.seekPath2(
        startNode,
        targetNode
      );
      ...
      if (roadNodeArr.length >= 2) {
        ...
        /* this.targetNode = roadNodeArr[roadNodeArr.length - 1].ele.setBGColor(
          "transparent",
          0.5
        ); */
      }
      ...
      this.move(roadNodeArr[roadNodeArr.length - 1].province);
    });

    this.player = new Player();
    this.addChild(this.player);

    MapRoadUtils.instance.updateMapInfo(
      mapData.mapWidth,
      mapData.mapHeight,
      mapData.nodeWidth,
      mapData.nodeHeight,
  ...
  move(province) {
    if (this.roadNodeArr && this.roadNodeArr.length > 0) {
      ...
      window["TweenMax"].to(this.player, duration, {
        ...
        onComplete: this.move.bind(this, province),
      });
    } else {
      this.player.stop();
      if (policy[province]) {
        console.log(province, policy[province]);
        const { name, high, mid, foreignEntry, low, underRisk, normal } =
          policy[province];
        alert(`${name}：
        高风险地区${high}个,
        中风险地区${mid}个,
        低风险地区${low}个,
        境外入境人员需要${foreignEntry},
        境内风险地区人员进入需要遵循：
        ${underRisk}
        其他地区人员进入需要遵循:${normal}
        `);

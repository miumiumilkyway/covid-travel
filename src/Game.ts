...
class Scroll {
  ...
  constructor(option) {
    let { content, maskWidth, maskHeight, contentWidth, contentHeight } =
...
class Zoom {
  ...
  constructor(config) {
    let { ele, value } = (this.$config = Object.assign(
      {},
      Zoom.defaultConfig,
      config
    ));
  ...
  zoom(e) {
    ...
    // 目标缩放值
    let targetZoom =
      e.wheelDelta <= 0 ? currentZoom - speed : currentZoom + speed;
